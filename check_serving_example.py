...
def main() -> None:
    ...
    with TemporaryDirectory(prefix="httk-serving-example-") as directory:
        ...
        for _ in range(2):
            subprocess.run([sys.executable, "api.py", "build"], cwd=root, check=True, timeout=60)
        sys.path.insert(0, directory)
        try:
            api = importlib.import_module("api")
            (root / "query.py").write_text(python[1], encoding="utf-8")
            output = subprocess.run(
                [sys.executable, "query.py"], cwd=root, check=True, capture_output=True, text=True, timeout=60
            )
            assert output.stdout.strip() == "ClNa -1.2", output.stdout
            with api.open_store() as store, socket.socket() as listener:
                listener.bind(("127.0.0.1", 0))
                port = listener.getsockname()[1]
                server = uvicorn.Server(uvicorn.Config(create_asgi_app(store), log_level="error"))
                ...
                try:
                    ...
                    linked_id = expected_item["relationships"]["structures"]["data"][0]["id"]
                    assert [(s["type"], s["id"]) for s in payload["included"]] == [("structures", linked_id)]
                    related = get("/v1/_httk_records", filter='structures.elements HAS "Na"')
                    assert [r["id"] for r in related["data"]] == [expected_item["id"]]
                    sorted_records = get("/v1/_httk_records", sort="-_httk_custom_formation_energy")["data"]
                    assert [r["attributes"]["_httk_custom_formation_energy"] for r in sorted_records] == [-0.8, -1.2]
                    (root / "remote_query.py").write_text(
                        python[2].replace("http://127.0.0.1:8080", base), encoding="utf-8"
                    )
                    output = subprocess.run(
    ...
    print(f"{page.name}: CIF/JSON import, reopen, relationships, and HTTP client passed")
