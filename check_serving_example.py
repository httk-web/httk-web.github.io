...
def main() -> None:
    """Verify the three-file import and serving quickstart over real HTTP."""
    ...
    with TemporaryDirectory(prefix="httk-serving-example-") as directory:
        ...
        for _ in range(2):
            subprocess.run([sys.executable, "build_db.py"], cwd=root, check=True, timeout=60)
        ...
        with (root / "server.log").open("w+") as log:
            ...
            try:
                ...
                for record in records:
                    (link,) = record["relationships"]["structures"]["data"]
                ...
                assert result["relationships"]["structures"]["data"][0]["id"] == structure["id"]
                related = get(
                    "/v1/_httk_records",
                    filter=f'_httk_relationships.structures.id HAS "{structure["id"]}"',
                )
    ...
    print(f"{page.name}: three-file import, deduplication, reopening without inputs, and HTTP relationships passed")
